...
use Dazzle\Uri\UriHelper;
use PHPUnit\Framework\TestCase;

/**
 * Tests for the Dazzle\Uri\UriHelper class.
 *
 * @since  1.0
 */
class UriHelperTest extends TestCase
{
	 ...
	 * @return  array
	 *
	 * @covers  Dazzle\Uri\UriHelper::parse_url
	 * @since   1.0
	 */
	public function testParse_Url()
	{
		$url = 'http://localhost/wordpress/index.php?p=1';
		...
		$url = 'http://localhost/wordpress/È';
		$expected = parse_url($url);

		// Fix up path for UTF-8 characters
		$expected['path'] = '/wordpress/È';
		$actual = UriHelper::parse_url($url);
		$this->assertEquals($expected, $actual, 'Line: ' . __LINE__ . ' Results should be equal');
